  ...
  describe('Full Update Flow Integration', () => {
    test('should complete full update cycle', async () => {
      ...
      autoUpdater.checkForUpdates.mockImplementation(() => {
        ...
        const checkingHandler = autoUpdater.on.mock.calls.find(c => c[0] === 'checking-for-update')?.[1]
        const availableHandler = autoUpdater.on.mock.calls.find(c => c[0] === 'update-available')?.[1]
      ...
      autoUpdater.downloadUpdate.mockImplementation(() => {
        const downloadedHandler = autoUpdater.on.mock.calls.find(c => c[0] === 'update-downloaded')?.[1]
  ...
  describe('Production Scenario Testing', () => {
    test('should handle high-load update server scenario', async () => {
      ...
      autoUpdater.checkForUpdates.mockImplementation(() => {
        ...
        return Promise.resolve({ updateInfo: { version: '2.0.0' } })
      })
